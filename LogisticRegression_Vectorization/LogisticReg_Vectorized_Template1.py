...
def hypothesisLogistic(X, coefficients, bias):
    ...
    # coefficients = coefficients.reshape(X.shape[0], 1)
    # print(coefficients.shape)
    predictedValues = np.dot(coefficients.T,X)+bias
...
def gradient_descent_log(bias, coefficients, alpha, X, Y, max_iter, X_val, y_val):
    ...
    for num in range(0, max_iter):
        ...
        # predictedY = predictedY.reshape(-1,1)
        # print(predictedY.shape)
        error = predictedY - Y
        gradient_bias = np.sum(error) / m
        gradient_coefficients = (X @ error.T) / m
        # print("shape: ", gradient_coefficients.shape)
        coefficients = coefficients - (alpha * gradient_coefficients)
...
def logisticRegression(X_train, y_train, X_validation, y_validation):
    ...
    coefficients = np.zeros(X_train.shape[0]).reshape(X_train.shape[0], 1)
...
def main():
    ...
    val_set_x_orig = np.transpose(val_set_x_orig)
